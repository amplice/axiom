...
pub fn docs_endpoints() -> Vec<serde_json::Value> {
    vec![
        ...
        serde_json::json!({"method":"POST","path":"/simulate","description":"Run headless simulation (supports optional goal_position/goal_radius). Trace frames include coyote/jump_buffer counters for jump diagnostics"}),
        ...
        serde_json::json!({"method":"POST","path":"/scripts/{name}/test","description":"Dry-run script with diagnostics. Returns entity_health, entity_alive, entity_vx/vy, entity_animation, defines_on_death, defines_init after one update() call"}),
        ...
        serde_json::json!({"method":"POST","path":"/entities/bulk","description":"Bulk mutate entities matching filter. Body: {filter: {tag?, component?, alive?, ids?}, mutations: {health_current?, add_tags?, contact_damage?, hitbox_active?, alive?, ...}}"}),
        serde_json::json!({"method":"GET","path":"/scripts/vars/diff","description":"Get script var changes since last diff call. Returns {changed, added, removed, snapshot_id}. First call establishes baseline."}),
        // Health check
